...
public class ElementHandler (private val jsonArray: JSONArray) {
    ...
    fun check() {
        /*
            Comprobamos que todos los elementos contienen datos conocidos.
        */
        ...
        for (item in jsonArray) {
            ...
            givenKeys.forEach {
                when (it) {
                    ...
                    "sessions" -> jsonObject.getJSONArray("sessions").forEach{ any ->
                        val innerJsonObject: JSONObject = any as JSONObject
                        val session = Session()
